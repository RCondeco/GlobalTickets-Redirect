...
class Url extends Conn
{
    ...
    {
        $stmt = $this->connect()->prepare('SELECT User_ID, Long_URL, Short_URL, ID FROM url WHERE User_ID = ?;');
    ...
    {
        $stmt = $this->connect()->prepare('UPDATE url SET Long_Url = ?, Short_URL = ? WHERE ID = ?');
